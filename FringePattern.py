...
class FringePattern:
    def __init__(self, width=800, height=600, f_sin=2, steps=4):
        ...
        self.sin_values = []
        self.freq = steps # number of time steps
        self.image = np.zeros((int(height), int(width), self.freq), dtype=np.uint8) # vector image
    ...
    def create_fringe_image(self):
        ...
        for n in range(self.freq): # phase shift of n=4
